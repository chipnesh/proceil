...
import me.chipnesh.proceil.service.MailService
import me.chipnesh.proceil.service.UserService
import me.chipnesh.proceil.service.dto.PasswordChangeDTO
import me.chipnesh.proceil.service.dto.UserValueObject
import me.chipnesh.proceil.web.rest.errors.ExceptionTranslator
import me.chipnesh.proceil.web.rest.vm.KeyAndPasswordVM
import me.chipnesh.proceil.web.rest.vm.ManagedUserVM
...
class AccountResourceIT {
    ...
    fun testRegisterDuplicateEmail() {
        ...
        userService.updateUser((UserValueObject(testUser4.get())))
    ...
    fun testGetCurrentSessions() {
        val user = UserModel()
        user.password = RandomStringUtils.random(60)
        user.login = "current-sessions"
        user.email = "current-sessions@example.com"

        userRepository.saveAndFlush(user)

        val token = PersistentToken()
        token.series = "current-sessions"
        token.user = user
        token.tokenValue = "current-session-data"
        token.tokenDate = LocalDate.of(2017, 3, 23)
    ...
    fun testInvalidateSession() {
        val user = UserModel()
        user.password = RandomStringUtils.random(60)
        user.login = "invalidate-session"
        user.email = "invalidate-session@example.com"

        userRepository.saveAndFlush(user)

        val token = PersistentToken()
        token.series = "invalidate-session"
        token.user = user
        token.tokenValue = "invalidate-data"
        token.tokenDate = LocalDate.of(2017, 3, 23)
        token.ipAddress = "127.0.0.1"
        token.userAgent = "Test agent"
